...
test("renders fetching data while shows is empty", async () => {
  mockFetchShow.mockResolvedValueOnce(mockData)
...
test("selecting each season from dropbown renders available episodes", async () => {
  mockFetchShow.mockResolvedValueOnce(mockData)
  ...
  userEvent.click(getAllByText(/season 1/i)[0]);
  userEvent.click(getByText(/season 2/i));
  
  await waitFor(() => {
    expect(getAllByTestId('episode')).toHaveLength(9);
  })
  
  userEvent.click(getAllByText(/season 2/i)[0]);
  ...
  userEvent.click(getAllByText(/season 3/i)[0]);
  userEvent.click(getByText(/season 4/i));

  await waitFor(() => {
    expect(getAllByTestId('episode')).toHaveLength(1);
  })
